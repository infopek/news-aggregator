...
async function loadArticles(base:FeedQuery,signal:AbortSignal){const result:ArticleSummary[]=[];let cursor:string|undefined;do{const page=await server.feed({...base,cursor},signal);result.push(...page.items);cursor=page.nextCursor??undefined}while(cursor&&base.includeHidden);return result}
