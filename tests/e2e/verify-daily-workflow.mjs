...
try {
  ...
  await page.locator('#source-filter').selectOption({ label: 'Metadata News' }); await page.locator('#saved-filter').selectOption('saved'); await page.locator('#feed-search').fill('climate'); await page.getByRole('button', { name: 'Apply filters' }).click(); await page.getByRole('link', { name: 'Climate science briefing', exact: true }).waitFor()
